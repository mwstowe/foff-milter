...
impl EmailAnonymizer {
    ...
    fn anonymize_email_addresses(&mut self, content: &str) -> String {
        ...
        email_re.replace_all(content, |caps: &regex::Captures| {
    ...
    fn get_anonymous_username(&mut self, username: &str) -> String {
        if let Some(replacement) = self.username_replacements.get(username) {
            ...
            self.username_replacements.insert(username.to_string(), replacement.clone());
            replacement
        }
    }
    ...
    fn get_anonymous_domain(&mut self, domain: &str) -> String {
        ...
        if domain.contains("gmail.com") || domain.contains("yahoo.com") || 
           domain.contains("hotmail.com") || domain.contains("outlook.com") {
        ...
        if let Some(replacement) = self.domain_replacements.get(domain) {
            ...
            self.domain_replacements.insert(domain.to_string(), replacement.clone());
            replacement
        }
    }
