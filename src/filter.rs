...
--boundary123--
    ...
    async fn test_sender_spoofing_extortion_detection() {
        ...
        let mut headers = HashMap::new();
        headers.insert("from".to_string(), "<marcy@baddomain.com>".to_string());
        headers.insert("to".to_string(), "<marcy@baddomain.com>".to_string());
        headers.insert(
            "received".to_string(),
        ...
        let context = MailContext {
            sender: Some("marcy@baddomain.com".to_string()),
            from_header: Some("<marcy@baddomain.com>".to_string()),
            recipients: vec!["marcy@baddomain.com".to_string()],
            headers,
        ...
        nytimes_headers.insert("to".to_string(), "mstowe@baddomain.com".to_string());
        nytimes_headers.insert(
            "received".to_string(),
        ...
        let nytimes_context = MailContext {
            sender: Some("mstowe+caf_=mstowe=baddomain.com@gmail.com".to_string()),
            from_header: Some("The New York Times <nytdirect@nytimes.com>".to_string()),
            recipients: vec!["mstowe@baddomain.com".to_string()],
...
/// Extract domain from DKIM signature (d= parameter)
fn extract_dkim_domain(dkim_signature: &str) -> Option<String> {
...
/// Extract domain from sender (From header or envelope sender)
...
fn extract_domain_from_email(email: &str) -> Option<String> {
    // Handle various email formats: "Name" <email@domain.com>, <email@domain.com>, email@domain.com
    let email_part = if email.contains('<') && email.contains('>') {
        // Extract email from angle brackets
        let start = email.find('<')?;
        let end = email.find('>')?;
        &email[start + 1..end]
    } else {
        email.trim()
