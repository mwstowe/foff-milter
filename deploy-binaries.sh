...
deploy_configs() {
    ...
    for server_config in "${SERVERS[@]}"; do
        ...
        ssh "$server" "
            ...
            # COMPLETELY WIPE rulesets directory to ensure clean deployment
            sudo rm -rf $remote_base_dir/rulesets/*
            
            # COMPLETELY WIPE features directory to ensure clean deployment  
            sudo rm -rf $remote_base_dir/features/*
            
            # Clean up any old backup files
            sudo find $remote_base_dir -name '*.old' -delete 2>/dev/null || true
...
check_vm_available() {
    ...
    if ping -c 1 -W 5 "$vm_fqdn" >/dev/null 2>&1; then
        if ssh -o ConnectTimeout=15 -o BatchMode=yes "$vm_fqdn" "echo 'VM ready'" >/dev/null 2>&1; then
